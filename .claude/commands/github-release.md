...
Read root `package.json` to get current version (e.g., "1.0.0"). Note: `backend/pyproject.toml` also has a version field that must stay in sync.
...
### Step 11: Confirm Checks

This monorepo has the following check scripts in the root `package.json`:
...
Run each check script in order. Stop on first failure.
...
**Sync backend version:** Update `backend/pyproject.toml` to match the new version:
```bash
cd backend && sed -i 's/^version = ".*"/version = "<new-version-without-v>"/' pyproject.toml && cd ..
```

Amend the version commit to include the backend version change:
```bash
...
1. **Phase 1 - Issue Creation:**
   - Title: "Add re-rank strategy selection"
   - Labels: enhancement
   - Version: minor (v1.1.0)
   - Creates `.plans/add-re-rank-strategy-selection.md`
   - Updates CHANGELOG.md
   - Creates GitHub issue #10
   - Generates commit message: `feat(chat): add re-rank strategy selection (#10)`

2. **Phase 2 - Release Automation:**
   - Remote check: Up to date
   - Runs typecheck (frontend + backend)... passed
   - Runs lint:frontend... passed
   - Runs test (frontend + backend)... passed
   - Runs build:frontend... passed
