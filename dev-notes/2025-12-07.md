...
| `backend/app/services/rag_service.py` | Added `classify_gemini_error()` function; Added retry logic with exponential backoff; **Disabled LangChain's internal retry (`max_retries=0`)** |
...
### Critical Fix: Disable LangChain's Internal Retry

**Problem discovered:** After initial implementation, the error handling still wasn't working. LangChain's `ChatGoogleGenerativeAI` has its own built-in retry mechanism that intercepts `ResourceExhausted` exceptions before our code can catch them.

Logs showed LangChain retrying internally:
```
Retrying langchain_google_genai.chat_models._chat_with_retry in 2.0 seconds...
...
**Solution:** Disable LangChain's internal retry by setting `max_retries=0`:
...
**Result:** Error now properly propagates to frontend and displays user-friendly message.
...
#### Added
- **Gemini API error classification** - Distinguishes between rate limit (retryable) and quota exhausted (non-retryable) errors
- **Custom retry logic** - Exponential backoff for transient rate limits (max 2 retries)
- **Global rate limiter** - Protects API key from being exceeded by multiple concurrent users (10 req/min)
- **User-friendly error messages** - Japanese error messages for quota/rate limit errors displayed to users
- **Error code mapping** - Frontend maps error codes to localized messages

#### Changed
- **Disabled LangChain's internal retry** (`max_retries=0`) - Allows custom error handling to work properly
- **Rate limiting strategy** - Now uses both global (API key protection) and per-IP (abuse prevention) limits

#### Fixed
- **Silent API failures** - Errors from Gemini API now properly propagate to frontend instead of being swallowed
- **Infinite retry loop** - LangChain was retrying quota exhausted errors indefinitely; now fails fast with user feedback

#### Evidence

![Quota exhausted error message displayed to user](../evidence/fix-token-outage.png)
