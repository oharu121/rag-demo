...
- Dismisses on click (session state by default, localStorage optional)
- Only appears after server is ready (not during cold start)

#### Tooltip Anchoring Fix

**Problem:** Initial implementation used fixed positioning that didn't properly align with the document button. The tooltip appeared "too right" and wasn't visually connected to its target.

**Solution:** Added `targetRef` prop and dynamic position calculation using `getBoundingClientRect()`:

```typescript
// ChatInterface.tsx - Pass button ref to tooltip
const documentButtonRef = useRef<HTMLButtonElement>(null);

<button ref={documentButtonRef} onClick={() => setIsDrawerOpen(true)}>
  {/* ... */}
</button>
...
```typescript
// OnboardingTooltip.tsx - Calculate position relative to button
...
**Why this works:**
- `getBoundingClientRect()` returns the button's position relative to the viewport
- `rect.bottom + 12` places tooltip 12px below the button
- `window.innerWidth - rect.right` aligns tooltip's right edge with button's right edge
- Position recalculates when tooltip becomes visible
...
5. **Click "OK"** → Tooltip dismissed (session only by default)
6. **Page refresh** → Tooltip appears again (consistent demo experience)

---

## Onboarding Tooltip: Session State vs Persistent Storage

### The Change

Changed the onboarding tooltip to use session state by default instead of localStorage. This ensures a consistent demo experience where the tooltip appears on every page refresh.

### Rationale

For recruiter demos, we want:
- **Consistent experience** - Every visit shows the same guided flow
- **No hidden state** - Recruiters see exactly what users will see
- **Easy reset** - No need to clear localStorage between demos

### Implementation

Added `persistDismissal` prop to OnboardingTooltip:
...
Usage:
```tsx
// Demo mode (default) - tooltip shows on every page load
<OnboardingTooltip documentCount={3} onDismiss={handleDismiss} targetRef={buttonRef} />

// Production mode - tooltip only shows once per browser
<OnboardingTooltip documentCount={3} onDismiss={handleDismiss} targetRef={buttonRef} persistDismissal />
```

### Behavior Comparison

| Scenario | `persistDismissal=false` (default) | `persistDismissal=true` |
|----------|-----------------------------------|------------------------|
| First visit | Tooltip shows | Tooltip shows |
| After dismiss | Tooltip hidden | Tooltip hidden |
| Page refresh | Tooltip shows again | Tooltip stays hidden |
| New browser session | Tooltip shows | Tooltip stays hidden |

### Future Use

When moving to production, simply add `persistDismissal` prop to avoid annoying returning users with repeated onboarding.