...
## Evaluation After Changes (Standard Strategy: 11/15)

### Results Summary

| Query | Result | Analysis |
|-------|--------|----------|
| 1-4 | ✓ | Correctly found exception rules |
| 5 (リモートワーク) | ✗ | RAG failed - returned general rule, missed 第19条 exception |
| 6 (結婚祝金差額) | ✗ | RAG failed - said "区別されていません" but 第18条 has different amount |
| 7 (通勤手当差額) | ✗ | RAG failed - found 50,000円 but missed 第12条 (20,000円) |
| 8 (利用できない福利厚生) | ✗ | RAG failed - said "含まれていません" but 第20-23条 has exclusions |
| 9-15 | ✓ | Correctly answered |

### Scoring System Assessment

**Accuracy: ~95%** - The 4 failures are **legitimate RAG failures**, not scoring problems:

- Query 5: Didn't retrieve 第19条（対象外）
- Query 6: Didn't retrieve 第18条（慶弔見舞金の特例）
- Query 7: Didn't retrieve 第12条（支給額の特例）
- Query 8: Didn't retrieve 第20-23条（各種特例）

**Conclusion:** Standard chunking (1000/200) scoring 73% on raw data is a reasonable baseline. The failures correctly identify where RAG couldn't find implicit exceptions.
...
### Question
Should we use pre-trained evaluation models (like RAGAS) instead of our custom scoring?

### Options Evaluated

#### 1. LLM-as-Judge (RAGAS, TruLens)
Uses an LLM to evaluate RAG outputs with metrics like:
- **Faithfulness**: Is the answer grounded in retrieved context?
- **Answer Relevancy**: Does it address the question?
- **Context Precision/Recall**: Did retrieval work?

Frameworks:
- [RAGAS](https://docs.ragas.io/en/stable/howtos/applications/align-llm-as-judge/) - Most popular
- [TruLens RAG Triad](https://modulai.io/blog/evaluating-rag-systems-with-synthetic-data-and-llm-judge/)
- [Mistral LLM-as-RAG-Judge](https://mistral.ai/news/llm-as-rag-judge)

#### 2. Computation-Based (Our Current Approach)
String matching for expected terms - deterministic and reproducible.

### Comparison

| Approach | Pros | Cons |
|----------|------|------|
| **Our method** | Fast, deterministic, no extra cost, easy to interpret (11/15) | Can't judge nuance, needs manual expected terms |
| **RAGAS** | Captures semantic correctness, no ground truth needed | Extra cost (~$0.01-0.05/query), inconsistent scores, LLM bias, 0-1 floats hard to interpret |

### Decision: Keep Current Approach

**Rationale:**
1. **Demo goal is comparing strategies** - Simple scoring is enough to show "parent-child > standard"
2. **RAGAS adds complexity** - Another dependency, API costs, score interpretation issues
3. **Deterministic is better for demos** - Same query always gives same score
4. **Clear communication** - "11/15 correct" is more compelling than "0.73 faithfulness"

**When RAGAS would make sense:**
- Production systems needing continuous monitoring
- When expected answers can't be defined upfront
- Evaluating open-ended generation quality

### References
- [RAG Evaluation 2025 Metrics](https://labelyourdata.com/articles/llm-fine-tuning/rag-evaluation)
- [RAGAS Documentation](https://docs.ragas.io/en/stable/howtos/applications/align-llm-as-judge/)
- [AWS Bedrock + RAGAS](https://aws.amazon.com/blogs/machine-learning/evaluate-amazon-bedrock-agents-with-ragas-and-llm-as-a-judge/)
