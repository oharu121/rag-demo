...
### Problem
The original evaluation summary was a minimal bottom-anchored bar showing just `73% (11/15 正解)` with a generic message. Users couldn't:
...
2. **Strategy-specific insights** - Explains *why* based on `.classmethod` analysis:
   ```
   💡 標準チャンクでは、例外規定が本文と分離されているため、
      複数ホップ推論や比較計算で必要な情報が取得されにくい傾向があります。
   ```
...
export const STRATEGY_INSIGHTS: Record<string, string> = {
  ...
  original_large_false:
    "大きなチャンクにより、一般規則と例外が同一チャンク内に保持され...",
...
### Problem
When clicking the "精度テストを実行" button, the progress display briefly showed "0/10" before correcting to "0/15" once the first `query_start` event arrived from the server. This caused a confusing visual jump.

### Root Cause
In `ChatInterface.tsx`, the evaluation progress was initialized with a hardcoded total of 10:
```typescript
setEvaluationProgress({ current: 0, total: 10 }); // ❌ Wrong - hardcoded value
```
...
#### 2. Show "準備中..." when total is unknown (`DatasetSelector.tsx`)
...
// After - show "準備中..." until we know the total
<span>
...
- Citations like `[福利厚生規程.md:119]` (single line number) were not styled in blue - only range format `[file:10-20]` worked
...
1. Citations with Japanese format `[出典: 福利厚生規程.md, 154-156行目]` weren't styled blue
...
1. Context headers used: `[出典: {filename}, {start_line}-{end_line}行目]`
2. Prompt instructed LLM to use: `[ファイル名:行番号]`
...
- `[福利厚生規程.md:154-156]` → blue citation (consistent format from backend)
- `**重要:**` → **重要:** (bold text, prompted by backend)
- Better paragraph spacing (`mb-3`), tighter bullet points (`space-y-1`)

---

## Simplified Markdown Rendering (Replaced react-markdown)

### Problem with react-markdown

The `react-markdown` approach had issues:
1. **Citations on separate lines** - react-markdown's paragraph component added `block` styling, causing citations after bullet points to appear on their own line
2. **List spacing too large** - `<ul>` and `<li>` elements with margins created excessive gaps
3. **Unnecessary complexity** - We only needed 2 features (citations + bold), not a full markdown parser

### Solution: Custom Regex-Based Rendering

Replaced `react-markdown` with a simple custom `renderContent()` function that handles only what we need.
...
1. **CSS `whitespace-pre-wrap`** handles line breaks and bullet formatting naturally - no need for markdown list parsing

2. **Single regex pass** - Combined pattern `(\[[^\]]+\])|(\*\*[^*]+\*\*)` matches both citations and bold in one pass

3. **`isCitation()` helper** - Validates bracketed text is actually a citation (not just any `[text]`):
   - Standard: `[file.md:10]` or `[file.md:10-20]`
   - Japanese: `[出典: file, X-Y行目]` (handles both `:` and `：` colons)

4. **No block elements** - Everything renders inline, so citations flow naturally within bullet points and paragraphs

### Why Not Use `marked` or Other Libraries?

Considered `marked` npm package but decided against it because:
1. Only 2 features needed (citations + bold)
2. `marked` would create block elements (same problem as react-markdown)
3. Custom solution is simpler and lighter (~50 lines vs adding a dependency)
4. CSS `whitespace-pre-wrap` handles the rest naturally
...
|-------|--------|
| `**bold text**` | **bold text** |
| `[file.md:10]` | <span style="color:blue">blue citation</span> |
| `[file.md:10-20]` | <span style="color:blue">blue citation</span> |
| `[出典: file, 10-20行目]` | <span style="color:blue">blue citation</span> |
| Line breaks | Preserved via `whitespace-pre-wrap` |
| `* bullet` | Shows as-is (LLM output, preserved by whitespace) |

### Files Changed

| File | Change |
|------|--------|
| `frontend/app/components/MessageBubble.tsx` | Replaced `renderContentWithMarkdown()` with simpler `renderContent()` |
| `frontend/package.json` | `react-markdown` can be removed (still in deps but unused) |

### Result

- Citations flow inline naturally (no unwanted line breaks)
- Bold text renders correctly
- Simpler code, fewer dependencies
- LLM's natural formatting (bullets, line breaks) preserved by CSS
